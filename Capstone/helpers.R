...
make_prediction <- function(phrase){
        ...
        one_check <- function(){
                suggestions <- models$Unigram$estimate[ grams$word, ] %>%
                        sort(decreasing = T) %>%
                        head(1) %>%
                        names()
                return(suggestions)
        ...
        two_check <- function(){
                suggestions <- models$Bigram$estimate[ bigram$bigram, ] %>%
                        sort(decreasing = T) %>%
                        head(1) %>%
                        names()
                return(suggestions)
        ...
        three_check <- function(){
                suggestions <- models$Trigram$estimate[ trigram$trigram, ] %>%
                        sort(decreasing = T) %>%
                        head(1) %>%
                        names()
                return(suggestions)
        ...
        if(length == 3){
                ...
                try(three_check(), silent = TRUE)
        } 
        
        else if(length == 2){
                
                try(two_check(), silent = TRUE) 
        } 
        
        else if(length == 1){
                try(one_check(), silent = TRUE)
        ...
        suggestions <- markovchainSequence(n = 1, models$Unigram$estimate)
        return(suggestions)
